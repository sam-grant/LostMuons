...
void Plotter::InitHistos() {
  ...
  for (int i=0; i < 2; i++) {
    ...
    //Labelling loop - 0: Lower, 1: Upper
  // Lost Muons
...
void Plotter::Run() {
  ...
  while( NextClTrTrEvent() ) {
    ...
    if(-3.2 < logEop && logEop < -2.4 && p > 2300) upper=true;
    if(-2.4 < logEop && logEop < -1.6 && p > 2300 ) lower=true;
    ...
    if ( lower ) { 
      ...
      Fill2DTr("mu_EoP_0",p,E);
      Fill2DTr("mu_pt_0",t,p);
      Fill1DTr("mu_y_track_0", TrY);
      Fill1DTr("mu_run_0",run);
      Fill1DTr("mu_energy_0",E);
      Fill1DTr("mu_radius_0",R);
      Fill2DTr("mu_xy_calo_0",cTrX,cTrY);
      Fill1DTr("mu_mom_0",p);
      Fill1DTr("mu_time_0",t);
      Fill2DTr("mu_xy_calo_fit_0",cTrX,cTrY);

    } 

    else if ( upper ) { 

      Fill2DTr("mu_EoP_1",p,E);
      Fill2DTr("mu_pt_1",t,p);
      Fill1DTr("mu_y_track_1", TrY);
      Fill1DTr("mu_run_1",run);
      Fill1DTr("mu_energy_1",E);
      Fill1DTr("mu_radius_1",R);
      Fill2DTr("mu_xy_calo_1",cTrX,cTrY);
      Fill1DTr("mu_mom_1",p);
      Fill1DTr("mu_time_1",t);
      Fill2DTr("mu_xy_calo_fit_1",cTrX,cTrY);
