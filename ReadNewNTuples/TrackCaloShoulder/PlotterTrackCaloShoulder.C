...
// Sam Grant                                             //
// August 2019                                           //
// Read allmuons branch, look at high momentum shoulder  //
///////////////////////////////////////////////////////////
...
void Plotter::InitHistos() {
  ...
  // cut, change the definition of *non* lost muons
  // cut = 0, normal; cut = 1, high energy cut; cut = 2, wide.
  for (int cut(0); cut<3; cut++) {
    
  // k=0, all particles; k=1, non lost muons; k=2, lost muons
  for (int k(0); k<3; k++) {
    // TODO: finish this bloody code
    plot1D("time_"+std::to_string(k)+std::to_string(cut),150*10,0,4200*150,"Decay Time [ns]","Entries");
