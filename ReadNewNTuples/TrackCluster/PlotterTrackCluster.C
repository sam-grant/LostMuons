...
void Plotter::Run() {
  ...
  while( NextallmuonsEvent() ) {
    ...
    for(int i=0; i<am->nmatches; i++) {
      ...
      // Calo number
      const int calo = am->cluCaloNum[i];
      // Track-cluster time difference
      const double dt = am->Tdiff[i];
      // Log(E/p)
      const double logEop = log(am->EovP[i]);
      // Backward decay vertices
      const double vTrX = am->decayvX[i];
      const double vTrY = am->decayvY[i];
      const double vTrZ = am->decayvZ[i];
      // Magic radius
      ...
      const double caloY = SetCaloY(calo, caloY_raw);
      // Radial difference
