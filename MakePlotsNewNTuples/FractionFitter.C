...
int main() {
  ...
  // Read inputs
  /* string trkInputFname = "../ReadNewNTuples/Track/RootFiles/TrackCandidate.root"; */
  /* string trkCluInputFname = "../ReadNewNTuples/TrackCluster/RootFiles/TrackClusterCandidate.root"; */
  // Insane older ROOT files
  string trkInputFname = "../OldRootFiles/merged_tracker_noVQ.root";
  string trkCluInputFname = "../OldRootFiles/merged_cluster_james.root";
  ...
  for (int stn = 12 ; stn < 19 ; stn = stn + 6) {
    ...
    string fracFname = "Plots/St"+to_string(stn)+"FracFit.png";
    string momRatioFname = "Plots/St"+to_string(stn)+"MomRatio.png";
     
    string title = "Stn"+to_string(stn)+";Track Momentum, p [MeV];Entries";
  
    TH1D *tr = (TH1D*)trkInput->Get(("St"+to_string(stn)+"_momentum").c_str()); // all tracks
    TH1D *n = (TH1D*)trkCluInput->Get(("St"+to_string(stn)+"_momentum_1").c_str()); // non lost muons
    TH1D *mu = (TH1D*)trkCluInput->Get(("St"+to_string(stn)+"_momentum_2").c_str()); // lost muons
