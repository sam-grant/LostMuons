...
int main() {
  ...
  for (int stn = 12 ; stn<19 ; stn = stn + 6) {
    ...
    if ( stn == 12 ) {
      ...
      sfMu = 1.72981;//1.8;//72981;//1.56875;// +/- 0.0385335 
    }
  
    string h1 = "St"+to_string(stn)+"_vertical60";
    string h2 = "St"+to_string(stn)+"_vertical60_2";

    string fname1;

    if (scale) {
      fname1 = "Plots/St"+to_string(stn)+"VerticalScaled.png";
    }
    else if (!scale) { 
      fname1 = "Plots/St"+to_string(stn)+"VerticalUnscaled.png";
    }

    string title1 = "Stn "+to_string(stn)+";Vertical Decay Vertex [mm];Entries";
 
    // string title2 = "Stn "+to_string(stn)+" | Sanity Check";

    /* TFile *f1 = TFile::Open("../ReadNewNTuples/Track/RootFiles/Track.root"); */
    /* TFile *f2 = TFile::Open("../ReadNewNTuples/TrackCluster/RootFiles/TrackCluster.root"); */
    ...
    // Alter this
    tr->GetXaxis()->SetRangeUser(-60,60);
