...
int main() {
  ...
  // No vertexQ
  string trackerFile = "../tracker/merged_tracker_combined.root";
  // Only track quality
  string clusterFile = "../cluster_master/merged_cluster_combine.root";
  string h0, h2, h3, fname1, fname2, title;

  if (!combined) {
    h0 = "St18_momentum"; // Tracker
    h2 = "St18_momentum_1"; // Non Lost Muons
    h3 = "St18_momentum_2"; // Lost Muons
    fname1 = "../plots/St18fracFit2.png";
    fname2 = "../plots/St18momRatio2.png";
    title = "Stn 18 | Fractional Fit Region; Track Momentum [MeV]; N";
  }
  else if(combined){
    h0 = "momentum"; // Trackers
    h2 = "momentum_1"; // Non Lost Muons
    h3 = "momentum_2"; // Lost Muons
    fname1 = "../plots/fracFit2.png";
    fname2 = "../plots/momRatio2.png";
    title = "Both Stations | Fractional Fit Region; Track Momentum [MeV]; N";

  }
  cout<<trackerFile<<endl;
  ...
  cout<<h3<<endl;
    ...
    cout << "##################################################" << endl;
    if(combined) cout << "Combined" << endl;
    if(!combined) cout << "Station 18" << endl;
    ...
    // tr->GetYaxis()->SetTitleOffset(1.1);
    tr->Draw();
    n->Draw("same");
    mu->Draw("same");
    tr->SetTitle(0);
    ...
    n->SetStats(0);
    mu->SetStats(0);
    tr->GetYaxis()->SetTitleSize(.05);
    tr->GetYaxis()->SetTitleOffset(0.4);
     tr->GetXaxis()->SetTitleSize(.05);
      tr->GetXaxis()->SetTitleOffset(0.9);
   
    tr->GetXaxis()->SetRange(bin2300,bin3000);
    tr->SetTitle(title.c_str());

    c2->cd(2);
    // ratio1->GetXaxis()->SetTitleSize(.08);
    ratio1->SetName("All Tracks / Non Lost Muons");
    ratio1->GetYaxis()->SetTitleSize(.05);
    ratio1->GetYaxis()->SetTitleOffset(0.4);
    ratio1->GetXaxis()->SetTitleSize(.05);
